...
import { getActivePatch, setActivePatch } from "@/lib/patchSystem";
import { defaultRealPatch } from "./realPatchData";
...
function App() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!active) {
        await setActivePatch(defaultRealPatch);
      }
      const res = await loadActivePatchToMemory();
      setHasPatch(res);
      setLoaded(true);
    }
    init();
